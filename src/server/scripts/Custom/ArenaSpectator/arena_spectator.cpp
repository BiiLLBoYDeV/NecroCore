...
public:
    ...
    static bool HandleSpectateCommand(ChatHandler* handler, char const* args)
    {
        ...
        if (bGround->isRated())
        {
            uint8 type = bGround->GetArenaType();
...
public:
    ...
    static bool OnGossipSelect(Player* player, Creature* creature, uint32 sender, uint32 action)
    {
        ...
        else if (action >= NPC_SPECTATOR_ACTION_2V2_GAMES && action < NPC_SPECTATOR_ACTION_3V3_GAMES)
        {
            ...
            ShowPage(creature, player, action - NPC_SPECTATOR_ACTION_2V2_GAMES, ARENA_TYPE_2v2, NPC_SPECTATOR_ACTION_2V2_GAMES);
        ...
        else if (action >= NPC_SPECTATOR_ACTION_3V3_GAMES && action < NPC_SPECTATOR_ACTION_2V2_GAMES)
        {
            ...
            ShowPage(creature, player, action - NPC_SPECTATOR_ACTION_3V3_GAMES, ARENA_TYPE_3v3, NPC_SPECTATOR_ACTION_3V3_GAMES);
            SendGossipMenuFor(player, DEFAULT_GOSSIP_MESSAGE, creature->GetGUID());
        }
        else if (action >= NPC_SPECTATOR_ACTION_5V5_GAMES && action < NPC_SPECTATOR_ACTION_SELECTED_PLAYER)
        {
            ...
            ShowPage(creature, player, action - NPC_SPECTATOR_ACTION_5V5_GAMES, ARENA_TYPE_5v5, NPC_SPECTATOR_ACTION_5V5_GAMES);
            SendGossipMenuFor(player, DEFAULT_GOSSIP_MESSAGE, creature->GetGUID());
        }
        else
        {
    ...
    static std::string GetGamesStringData(Battleground* team, uint32 mmr, uint32 mmrTwo)
    {
    ...
    static ObjectGuid GetFirstPlayerGuid(Battleground* team)
    {
        for (Battleground::BattlegroundPlayerMap::const_iterator itr = team->GetPlayers().begin(); itr != team->GetPlayers().end(); ++itr)
            if (Player* player = ObjectAccessor::FindPlayer(itr->first))
                return itr->first;
    ...
    static void ShowPage(Creature* creature, Player* player, uint16 page, uint32 type, uint32 action)
    {
        uint32 firstTeamId = 0;
        uint16 TypeOne = 0;
        uint16 TypeTwo = 0;
        ...
        for (uint8 i = 0; i <= MAX_BATTLEGROUND_TYPE_ID; ++i)
        {
            ...
            for (BattlegroundContainer::const_iterator itr = arenas->m_Battlegrounds.begin(); itr != arenas->m_Battlegrounds.end(); ++itr)
            {
                ...
                if (type == 1 && arena->GetArenaType() == ARENA_TYPE_2v2)
                {
                    TypeOne++;
                ...
                else if (type == 2 && arena->GetArenaType() == ARENA_TYPE_3v3)
                {
                    TypeTwo++;
                ...
                else if (type == 3 && arena->GetArenaType() == ARENA_TYPE_5v5)
                {
                    TypeThree++;
        ...
        if (haveNextPage)
        {
            ...
            else if (action == NPC_SPECTATOR_ACTION_5V5_GAMES)
                AddGossipItemFor(player, GOSSIP_ICON_DOT, "Next..", GOSSIP_SENDER_MAIN, NPC_SPECTATOR_ACTION_5V5_GAMES + page + 1);
        }
        AddGossipItemFor(player, GOSSIP_ICON_DOT, "<-Back", GOSSIP_SENDER_MAIN, NPC_SPECTATOR_ACTION_BACK);
        return;
    }
